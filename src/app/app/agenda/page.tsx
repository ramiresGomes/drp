...
export default async function AgendaPage({
  ...
  const [scale, events, links] = await Promise.all([
    ...
    prisma.regionalEvent.findMany({
      ...
      include: { type: true, attendances: { where: { personId: person.id } } },
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-sm text-muted-foreground">
        Evento obrigatório aparece para toda a regional. Atendimento institucional só entra na sua escala se houver
        vínculo. Justificativa tem prazo padrão de dois dias antes, configurável na série.
      </p>
